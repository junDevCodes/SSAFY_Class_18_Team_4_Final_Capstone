...
class CategoryModelTest(TestCase):
    ...
    def test_category_unique_name(self):
        ...
        with self.assertRaises(Exception):
            Category.objects.create(name="과일/견과", slug="fruits-nuts-2")
...
class ProductModelTest(TestCase):
    ...
    def test_product_discount_range_constraint(self):
        """할인율이 0-100 범위인지 테스트"""
        # 정상 범위
        product = Product.objects.create(
            name="할인 상품",
        ...
        self.assertEqual(product.discount, 50)

    def test_product_default_values(self):
        """제품의 기본값이 올바르게 설정되는지 테스트"""
        product = Product.objects.create(
            name="기본값 테스트",
            price=1000,
            image_url="https://example.com/test.jpg"
        ...
        self.assertFalse(product.is_best)
