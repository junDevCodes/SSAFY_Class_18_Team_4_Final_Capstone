...
class ConfidenceWeights:
    ...
    근거: Netflix Prize (2009)
    - C_ui = 1 + α × log(1 + r_ui)
    - α는 희소 데이터에서 10~20, 대규모에서 40~100
    """
    ...
    def calculate_confidence_batch(self, scores: np.ndarray) -> np.ndarray:
        """배치 처리"""
        return 1.0 + self.alpha * np.log1p(scores)
...
class HybridWeights:
    ...
    근거: Netflix Prize (2009), Amazon (2020~2025)
    - 기본 비율: CBF 0.7 + CF 0.3
    - 희소 데이터에서 CBF 의존도 높임
    - 상호작용 증가 시 CF로 전환
    """
    # 상호작용 수 기준 CBF:CF 비율
...
def print_weight_config():
    ...
    print(f"  • 공식: C = 1 + {CONFIDENCE_WEIGHTS.alpha} × log(1 + score)")
...
def test_weight_configurations():
    ...
    # 수정된 가중치: view=0.1, cart=2.0, order=5.0
